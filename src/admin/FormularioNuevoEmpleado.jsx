...
function FormularioNuevoEmpleado({ onContinuar }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      employee_cedula: parseInt(datos.cedula),
      employee_first_name: datos.primerNombre,
      employee_second_name: datos.segundoNombre.trim() || null,
      employee_first_lastname: datos.apellido,
      employee_second_lastname: datos.segundoApellido.trim() || null,
      employee_email: datos.correo,
      employee_home_telephone: datos.telefonoCasa.trim() || null,
      employee_telephone: datos.telefono,
      employee_emergency_telephone: datos.telefonoEmergencia.trim() || null,
      employee_home_address: datos.direccion.trim() || null
    };

    console.log('📤 Enviando POST employees:', payload);
    ...
    try {
      ...
      console.log('📥 Respuesta POST employees:', result);
      ...
      if (!result.success) {
        throw new Error(result.message || 'Error en la creación del empleado');
      }

      displayMessage('✅ Empleado creado correctamente', 'success');
      ...
      displayMessage(`No se pudo crear el empleado: ${err.message}`);
    } finally {
      setCargando(false);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
              <div>
                ...
                <input 
                  className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-950"
                  name="telefonoCasa"
                  placeholder="Teléfono de casa (opcional)"
